...
SELECT NAME, SSN, DECODE(SUBSTR(SSN, 8, 1), 1, '남자', 2, '여자', '확인불가')
FROM TBL_INSA;
...
--   함수명 : FN_PAY(기본급, 수당)
SELECT BASICPAY "기본급", SUDANG "수당", (BASICPAY*12)+SUDANG "급여"
FROM TBL_INSA;
...
BEGIN
    ...
    V_RESULT := LTRIM(TO_CHAR(V_SALARY, 'L999,999,999'));
...
SELECT IBSADATE "입사일", TRUNC(((SYSDATE - IBSADATE)/365), 1)"근무년수"
FROM TBL_INSA;
