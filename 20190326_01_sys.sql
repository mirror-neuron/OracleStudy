...
SELECT 1000/23
FROM DUAL;
--==>> 43.47826086956521739130434782608695652174(정수/정수도 실수로 표현됨)
...
FROM DBA_USERS;
--==>> FROM(메모리, DAB_USERS 쿼리 전부 메모리로 끌어올림), SELECT(CPU)
/*
--==>> 위와 쓰이는 메모리 용량이 같다
SELECT USERNAME, ACCOUNT_STATUS, USER_ID, USER_PASSWORD 
FROM DBA_USERS;
*/
/*
SYS	OPEN
...
CREATE TABLESPACE TBS_EDUA                      -- CREATE 유형 개체명 → 생성(이하는 모두 옵션)
...
-- 물리적인 경로에 디렉터리(TESTORADATA) 폴더 생성할 것.
...
--○ 오라클 사용자 계정 생성
CREATE USER jsy IDENTIFIED BY java006$
DEFAULT TABLESPACE TBS_EDUA;
--> jsy 라는 사용자 계정을 만들겠다. (생성하겠다.)
--  이 계정을 통해 생성하는 오라클 객체는(세그먼트들은)
--  기본적으로 TBS_EDUA 라는 테이블 스페이스에 생성할 수 있도록 설정하겠다.
--==>> User JSY이(가) 생성되었습니다.


--※ 생성된 오라클 사용자 계정(jsy)을 통해
--   접속을 시도해 보았으나 접속 불가.
--   → 『create session』 권한이 없기 때문에...


--○ 생성된 오라클 사용자 계정(jsy)에 
--   서버 접속이 가능할 수 있도록 create session 권한 부여 → sys 가...
GRANT CREATE SESSION TO jsy;
--==>> Grant을(를) 성공했습니다.


--○ 생성된 오라클 사용자 계정(KHJ)의 
...
GRANT CREATE TABLE TO JSY;
--==>> Grant을(를) 성공했습니다.


--○ 생성된 오라클 사용자 계정(JSY)에
--   테이블스페이스(TBS_EDUA)에서 사용할 수 있는 공간(할당량→QUOTA)
--   의 크기를 무제한으로 지정.(구조적인 변경→ ALTER)
ALTER USER JSY
